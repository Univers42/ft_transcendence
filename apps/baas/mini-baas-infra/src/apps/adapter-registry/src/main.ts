...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Adapter Registry')
    .setDescription('Encrypted database credential store with tenant isolation')
    .setVersion('2.0.0')
  ...
  Logger.log(`adapter-registry listening on :${port}`, 'Bootstrap');
}
