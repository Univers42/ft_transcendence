...
|---|---|---|---|
| O1 | Internal service token is a **static shared secret** (plaintext header) | MED (**HMAC IMPLEMENTED**, default static) | **Per-request HMAC shipped + live-proven** under `SERVICE_TOKEN_MODE=hmac`: `X-Service-Auth: v1.<ts>.<hex hmac-sha256(token, ts\nMETHOD\nPATH\nsha256(body))>` (±120 s skew, body/path/method-bound, token never on the wire; plain tokens REJECTED in hmac mode). Byte-identical signing in Go (`shared.ComputeServiceSignature`)/Rust (`service_auth.rs`)/TS (`service-auth.ts`) with shared golden vectors. Live flip verified: Rust→Go and TS→Go signed traffic 200/201; static + bogus-sig rejected 401; valid sig authenticates. **Default stays `static`** because ops tooling (m-series/seed scripts) + `schema-service` still send static tokens — rollout = migrate those callers, then set `SERVICE_TOKEN_MODE=hmac` stack-wide. The TS↔TS permission-engine hop keeps a (now timing-safe) static compare by design. |
